...
window.CKKS_Sample = async function () {
    ...
    if (answer) {
        var memCkks = performance.memory.usedJSHeapSize / Math.pow(1000, 2) + 'MB';
        console.log(`${performance.memory.usedJSHeapSize / Math.pow(1000, 2)} MB`);

        Seal = require('node-seal');
        ...
        console.log(`${performance.memory.usedJSHeapSize / Math.pow(1000, 2)} MB`);
        var memCkks = parseFloat((performance.memory.usedJSHeapSize / Math.pow(1000, 2)).toString()) - parseFloat(memCkks.toString());
        var memCkks = parseFloat((memCkks / 1024).toFixed(4));
        result += ',' + memCkks.toString();
        return result;
    }
